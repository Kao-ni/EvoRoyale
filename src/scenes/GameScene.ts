...
type StaticSprite = Phaser.Types.Physics.Arcade.SpriteWithStaticBody;
...
export class GameScene extends Phaser.Scene {
  ...
  private xpOrbs!: Phaser.Physics.Arcade.StaticGroup;
  private walls!: Phaser.Physics.Arcade.StaticGroup;
  private generators!: Phaser.Physics.Arcade.StaticGroup;
  ...
  private resourceTitle!: Phaser.GameObjects.Text;
  ...
  private swordGuard!: Phaser.GameObjects.Rectangle;
  private toolSprite!: Phaser.GameObjects.Image;
  private buildGhost!: Phaser.GameObjects.Graphics;
  private restartButton?: Phaser.GameObjects.Text;
  private safeZoneGraphics!: Phaser.GameObjects.Graphics;
  private dangerZoneOverlay!: Phaser.GameObjects.Graphics;
  private safeZoneMaskGraphics!: Phaser.GameObjects.Graphics;
  private safeZoneMask!: Phaser.Display.Masks.GeometryMask;
  private healthBars!: Phaser.GameObjects.Graphics;
  private cooldownBar!: Phaser.GameObjects.Graphics;
  private hotbarPanel!: Phaser.GameObjects.Graphics;
  private hotbarTexts: Phaser.GameObjects.Text[] = [];
  private buildMenuText!: Phaser.GameObjects.Text;
  private playerState: PlayerProgress = createPlayerProgress();
  private facing = new Phaser.Math.Vector2(1, 0);
  private primaryActionQueued = false;
  private swingAngleOffset = 0;
  private toolSwingOffset = 0;
  private selectedTool: ToolId = "sword";
  private selectedBuildable: BuildableId = "wall";
  private zoneCenter = new Phaser.Math.Vector2(WORLD.width / 2, WORLD.height / 2);
  private entitySequence = 0;
  private lastAttackAt = -Infinity;
  private lastToolUseAt = -Infinity;
  private lastBuildAt = -Infinity;
  private lastBotContactAt = -Infinity;
  private nextZoneDamageAt = 0;
  private zoneStartAt = 0;
  private currentZoneRadius: number = SAFE_ZONE.startRadius;
  private zoneFrozen = false;
  private playerSpeed: number = WORLD.playerSpeed;
  private playerRadius: number = WORLD.playerRadius;
  ...
  private drawTreeTexture(key: string, damaged: boolean): void {
    if (this.textures.exists(key)) {
      return;
    }
  ...
  private createStoneTextures(): void {
  ...
  private drawStoneTexture(key: string, cracked: boolean): void {
    if (this.textures.exists(key)) {
      return;
    }
  ...
  private createToolTextures(): void {
  ...
  private createChipTexture(key: string, fillColor: number, strokeColor: number): void {
    const graphics = this.make.graphics({ x: 0, y: 0 }, false);
  ...
  private createGeneratorTexture(): void {
  ...
  private applyLevelGrowth(): void {
    ...
    const scale = Math.min(
      1 + level * LEVEL_GROWTH.scalePerLevel,
      LEVEL_GROWTH.maxScale,
    );
    ...
    const speedFactor = Math.max(
      1 - level * LEVEL_GROWTH.speedPenaltyPerLevel,
      LEVEL_GROWTH.minSpeedFactor,
    );
  ...
  private spawnTree(x: number, y: number): void {
    const tree = this.trees.create(x, y, "tree") as StaticSprite;
  ...
  private spawnStones(count: number): void {
    for (let i = 0; i < count; i += 1) {
      const point = this.getRandomArenaPoint(120);
  ...
  private spawnStone(x: number, y: number): void {
    const stone = this.stones.create(x, y, "stone") as StaticSprite;
  ...
  private spawnXpOrbs(count: number): void {
    for (let i = 0; i < count; i += 1) {
  ...
  private spawnXpOrb(x: number, y: number, value: number): void {
    const orb = this.xpOrbs.create(x, y, "xp-orb") as StaticSprite;
  ...
  private spawnBots(count: number): void {
    for (let i = 0; i < count; i += 1) {
      ...
      const bot = this.bots.create(point.x, point.y, "bot") as DynamicSprite;
  ...
  private getRandomArenaPoint(padding: number): Phaser.Math.Vector2 {
    return new Phaser.Math.Vector2(
      Phaser.Math.Between(padding, WORLD.width - padding),
      Phaser.Math.Between(padding, WORLD.height - padding),
    );
  }
  ...
  private applyBotLevelGrowth(bot: DynamicSprite, progress: any, prevIndex?: number): void {
    ...
    try {
      (bot.body as Phaser.Physics.Arcade.Body).setCircle(botRadius, bodyOffset, bodyOffset);
    } catch (e) {
    ...
    const botTool = bot.getData("toolSprite") as Phaser.GameObjects.Image | undefined;
    if (botTool) {
      const baseW = 92;
      const baseH = 50;
  ...
  private tryBotMineTarget(
    time: number,
    bot: DynamicSprite,
    group: Phaser.Physics.Arcade.StaticGroup,
    targetType: "tree" | "stone" | "wall",
  ): void {
    const lastToolUseAt = (bot.getData("lastToolUseAt") as number) ?? -Infinity;
    ...
    let closest: StaticSprite | undefined;
    let closestDistance = Number.POSITIVE_INFINITY;
    const dir = (bot.getData("direction") as Phaser.Math.Vector2) ?? new Phaser.Math.Vector2(1, 0);
    const aimAngle = dir.angle();

    group.children.each((child) => {
      const target = child as StaticSprite;
    ...
    this.time.delayedCall(250, () => {
      if (bot.active) bot.setData("equippedTool", undefined);
    });

    bot.setData("lastToolUseAt", time);
    const hp = (closest.getData("hp") as number) - BALANCE.toolDamage;
    ...
    if (hp > 0) {
      this.updateMineableDamageTexture(closest, targetType as any, hp);
      return;
    }
    ...
    const value = closest.getData("resourceValue") as number;
    closest.destroy();
    this.showBrokenMineable(x, y, targetType as any);

    const progress = bot.getData("progress") as any;
  ...
  private pickBotTool(bot: DynamicSprite, progress: PlayerProgress): ToolId {
    const playerThreat = this.getClosestLivingTarget(
      bot,
      this.player.x,
      this.player.y,
      this.playerRadius,
    ...
    if (
      (playerThreat && playerThreat.distance <= BALANCE.toolRange + WORLD.botRadius + this.playerRadius) ||
  ...
  private getClosestMineableInRange(
    bot: DynamicSprite,
    group: Phaser.Physics.Arcade.StaticGroup,
  ): { target: StaticSprite; distance: number } | undefined {
    let closest: { target: StaticSprite; distance: number } | undefined;
  ...
  private getClosestLivingTarget(
    bot: DynamicSprite,
    x: number,
    y: number,
    radius: number,
  ...
  private getClosestLivingBot(
    bot: DynamicSprite,
    maxDistance: number,
  ): { target: DynamicSprite; distance: number } | undefined {
    let closest: { target: DynamicSprite; distance: number } | undefined;

    this.bots.children.each((child) => {
      const other = child as DynamicSprite;
  ...
  private tagEntity(entity: Phaser.GameObjects.GameObject, entityType: string): void {
  ...
  private updateBuildGhost(): void {
    ...
    const blocked =
      this.isBuildPositionBlocked(position.x, position.y) ||
  ...
  private trySwordAttack(time: number): void {
    ...
    this.bots.children.each((child) => {
      const bot = child as DynamicSprite;
  ...
  private showSwordSwing(range: number, aimAngle: number): void {
    ...
    for (let i = 0; i <= steps; i += 1) {
      ...
      arc.lineTo(
        this.player.x + Math.cos(angle) * outerRadius,
        this.player.y + Math.sin(angle) * outerRadius,
      );
    }
  ...
  private isInSwordSwing(
    target: DynamicSprite,
    range: number,
    aimAngle: number,
  ): boolean {
  ...
  private damageBotWithSword(bot: DynamicSprite): void {
    const combat = bot.getData("combat") as CombatTarget;
    ...
    bot.setData(
      "direction",
      new Phaser.Math.Vector2(this.player.x - bot.x, this.player.y - bot.y).normalize(),
    );
  ...
  private killBot(
    bot: DynamicSprite,
    reward: { progress: PlayerProgress; x: number; y: number } | null,
  ): void {
    if (bot.getData("dead")) {
      return;
    }
    ...
    for (let i = 0; i < 3; i += 1) {
      this.spawnXpOrb(
        bot.x + Phaser.Math.Between(-18, 18),
        bot.y + Phaser.Math.Between(-18, 18),
        BALANCE.xpOrbValue,
      );
    }
  ...
  private tryMineTarget(
    ...
    if (!target) {
      if (targetType === "tree") this.showToast("Aim at a tree");
      else if (targetType === "stone") this.showToast("Aim at stone");
      else this.showToast("Aim at a wall");
      return;
    }

    const hp = (target.getData("hp") as number) - BALANCE.toolDamage;
    ...
    if (targetType === "tree") {
      const value = target.getData("resourceValue") as number;
  ...
  private findMineableTarget(
    group: Phaser.Physics.Arcade.StaticGroup,
  ): StaticSprite | undefined {
    let closest: StaticSprite | undefined;
    let closestDistance = Number.POSITIVE_INFINITY;
    const aimAngle = this.facing.angle();
  ...
  private updateMineableDamageTexture(
    target: StaticSprite,
    targetType: "tree" | "stone" | "wall",
    hp: number,
  ): void {
  ...
  private showToolStrike(
    ...
    arc.arc(
      this.player.x,
      this.player.y,
      BALANCE.toolRange + this.playerRadius,
      angle - 0.62,
      angle + 0.32,
      false,
    );
  ...
  private playToolSwing(targetType: "tree" | "stone" | "wall"): void {
  ...
  private tryPrimaryAction(time: number): void {
  ...
  private tryBuild(time: number, requested = false): void {
  ...
  private getBuildPosition(): Phaser.Math.Vector2 {
    ...
    return new Phaser.Math.Vector2(
      Phaser.Math.Clamp(snappedX, WORLD.wallSize, WORLD.width - WORLD.wallSize),
      Phaser.Math.Clamp(snappedY, WORLD.wallSize, WORLD.height - WORLD.wallSize),
    );
  }

  private isBuildPositionBlocked(x: number, y: number): boolean {
    const structures = [
  ...
  private placeBuildable(x: number, y: number, buildableId: BuildableId): void {
    if (buildableId === "wall") {
      const wall = this.walls.create(x, y, "wall") as StaticSprite;
    ...
    const generator = this.generators.create(x, y, "generator") as StaticSprite;
  ...
  private updateGenerators(time: number): void {
    this.generators.children.each((child) => {
      const generator = child as StaticSprite;

      if (!generator.active) {
        return true;
      }

      const nextResourceAt = generator.getData("nextResourceAt") as number;
      const remainingTreeSpawns = (generator.getData("remainingTreeSpawns") as number) ?? 0;
      ...
      generator.setData(
        "nextResourceAt",
        time + BUILDABLES.generator.resourceIntervalMs,
      );
  ...
  private getGeneratorSpawnPoint(generator: StaticSprite): Phaser.Math.Vector2 {
    const angle = Phaser.Math.FloatBetween(0, Math.PI * 2);
    const distance = Phaser.Math.Between(54, 78);

    return new Phaser.Math.Vector2(
      Phaser.Math.Clamp(
        generator.x + Math.cos(angle) * distance,
        WORLD.wallSize,
        WORLD.width - WORLD.wallSize,
      ),
      Phaser.Math.Clamp(
        generator.y + Math.sin(angle) * distance,
        WORLD.wallSize,
        WORLD.height - WORLD.wallSize,
      ),
    );
  }
  ...
  private updateBots(time: number): void {
    this.bots.children.each((child) => {
      ...
      const progress = (bot.getData("progress") as any) ?? createPlayerProgress();
      ...
      const lastAttackAt = (bot.getData("lastAttackAt") as number) ?? -Infinity;
      ...
      if (selectedTool === "sword" && attackReady) {
        ...
        // Attack nearest bot
        if (time - (bot.getData("lastAttackAt") as number) >= BALANCE.attackCooldownMs) {
          this.bots.children.each((maybe) => {
            const other = maybe as DynamicSprite;
            ...
            const d = Phaser.Math.Distance.Between(bot.x, bot.y, other.x, other.y);
              ...
              bot.setData("lastAttackAt", time);
              bot.setData("equippedTool", "sword");
              const combat = other.getData("combat") as CombatTarget;
              const died = applySwordHit(combat);
              ...
              other.setData(
                "direction",
                new Phaser.Math.Vector2(bot.x - other.x, bot.y - other.y).normalize(),
              );
      ...
      const retargetAt = bot.getData("retargetAt") as number;

      if (time >= retargetAt) {
        if (selectedTool === "build") {
          const direction = bot.getData("direction") as Phaser.Math.Vector2;
      ...
      const botTool = bot.getData("toolSprite") as Phaser.GameObjects.Image | undefined;
      const equipped = bot.getData("equippedTool") as string | undefined;
      if (botTool) {
        if (selectedTool === "build") {
          botTool.setVisible(false);
        } else if (equipped === "axe" || equipped === "pickaxe") {
          const textureKey = equipped === "axe" ? "axe-tool" : "pickaxe-tool";
          const angle = direction.angle() + (bot.getData("toolSwingOffset") as number ?? 0);
  ...
  private pickBotDirection(bot: DynamicSprite): Phaser.Math.Vector2 {
    const now = this.time.now;

    // helper: does moving along dir by lookahead put us outside safe zone?
    const willExitSafeZone = (dir: Phaser.Math.Vector2, lookahead = 220) => {
    ...
    const detAt = (bot.getData("detAt") as number) ?? 0;
    if (agro && now < detAt) {
      ...
      if (agro && typeof (agro as any).x === "number" && (agro as any).active) {
        const target = agro as DynamicSprite;
    ...
    const outsideZone = isOutsideSafeZone(
      bot.x,
      bot.y,
      this.zoneCenter.x,
      this.zoneCenter.y,
      this.currentZoneRadius,
    );
    ...
    let nearest: { obj: Phaser.Types.Physics.Arcade.SpriteWithDynamicBody | DynamicSprite; d: number } | null = null;
    ...
    this.bots.children.each((child) => {
      const other = child as DynamicSprite;
      if (!other.active || other.getData("dead") || other === bot) return true;
      const d = Phaser.Math.Distance.Between(bot.x, bot.y, other.x, other.y);
      if (d > detectionRadius) return true;
      if (!nearest || d < nearest.d) nearest = { obj: other, d };
      return true;
    });
    ...
    for (let i = 0; i < 6; i += 1) {
      ...
      if (!willExitSafeZone(candidate)) return candidate;
    }
  ...
  private shiftZoneBackOnDeath(): void {
    if (this.zoneFrozen) {
      return;
    }

    this.zoneStartAt = Math.min(
      this.time.now,
      this.zoneStartAt + SAFE_ZONE.deathReliefMs,
    );
  }

  private updateSafeZone(time: number): void {
    if (!this.zoneFrozen) {
    ...
    this.safeZoneMaskGraphics.fillCircle(
      this.zoneCenter.x,
      this.zoneCenter.y,
      this.currentZoneRadius,
    );
    ...
    this.safeZoneGraphics.strokeCircle(
      this.zoneCenter.x,
      this.zoneCenter.y,
      Math.max(8, this.currentZoneRadius - 10),
    );
  ...
  private damagePlayer(amount: number): void {
    ...
    const combat = this.player.getData("combat") as CombatTarget | undefined;
    if (combat) {
  ...
  private healBotAfterKill(bot: DynamicSprite): void {
    const combat = bot.getData("combat") as CombatTarget | undefined;
  ...
  private collectXp = (
    ...
    xpObject: ArcadeCallbackObject,
  ): void => {
    const orb = xpObject as StaticSprite;
  ...
  private drawAttackCooldownBar(time: number): void {
    const width = 58;
    const height = 8;
    ...
    const healthRatio = Phaser.Math.Clamp(
      this.playerState.hp / PLAYER_MAX_HP,
      0,
      1,
    );
    const remainingMs = Math.max(
      0,
      this.lastAttackAt + BALANCE.attackCooldownMs - time,
    );
    const remainingRatio = Phaser.Math.Clamp(
      remainingMs / BALANCE.attackCooldownMs,
      0,
      1,
    );
    ...
    this.cooldownBar.fillRoundedRect(
      x + 2,
      healthY + 2,
      Math.max(0, (width - 4) * healthRatio),
      height - 4,
      3,
    );
  ...
  private updateHotbar(): void {
    const slots: Array<{ id: ToolId; label: string }> = [
      { id: "sword", label: "1\nSword" },
      { id: "axe", label: "2\nAxe" },
      { id: "pickaxe", label: "3\nPick" },
      { id: "build", label: "4\nBuild" },
  ...
  private getRemainingBots(): number {
  ...
  private popText(x: number, y: number, message: string, color: string): void {
    const text = this.add
      .text(x, y, message, {
        fontFamily: "Arial, sans-serif",
        fontSize: "17px",
